      ...
      <div className="px-4 pt-2 pb-4 space-y-2">
        ...
        {belowMin && (
          <div className="rounded-2xl bg-amber-50 p-3 text-[12px] text-amber-800">
            Минимальный заказ — {formatPrice(MIN_ORDER_AMOUNT)}. Добавьте ещё на{" "}
            <strong>{formatPrice(MIN_ORDER_AMOUNT - subtotal)}</strong>.
          </div>
        )}
      </div>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      <span className={bold ? "text-[15px] font-semibold text-ink-900" : "text-[14px] text-ink-600"}>
        {label}
      </span>
      <span className={bold ? "text-[18px] font-extrabold text-ink-900" : "text-[14px] text-ink-900"}>
        {value}
      </span>
    </div>
  );
